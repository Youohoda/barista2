...
export default async function handler(req,res){
 try{const u=await requireUser(req);if(!owner(u))return json(res,403,{error:'OWNER_ONLY'});await db();
  if(req.method==='GET'){const users=await User.find().sort({createdAt:-1}).limit(100).lean();return json(res,200,{owner:true,ownerEmail:OWNER_EMAIL,codes:ALL_CODES,users:users.map(x=>({email:x.email,plan:x.unlimited?'god':x.plan||'free',unlimited:!!x.unlimited,premiumExpiresAt:x.premiumExpiresAt,dailyUsed:x.dailyUsed||0,createdAt:x.createdAt}))});}
