...
// One row per Autonomous Agent Engine run (api/_agent-engine.js + api/agent-run.js).
// `transitions` is the real state-machine history (QUEUED→PLANNING→IMPLEMENTING→
// TESTING→FIXING→REVIEWING→COMPLETED|FAILED) — appended to as runAgentLoop emits
// events, never written all at once after the fact, so a run that crashes mid-way
// still has a truthful partial history instead of nothing. `checkpointFiles` stores
// the pre-run snapshot needed for rollback (see rollbackToCheckpoint).
const AgentRunTransitionSchema = new mongoose.Schema({
...
const AgentRunSchema = new mongoose.Schema({
  ...
  status: { type: String, enum: ['QUEUED', 'PLANNING', 'IMPLEMENTING', 'TESTING', 'FIXING', 'REVIEWING', 'COMPLETED', 'FAILED', 'CANCELLED'], default: 'QUEUED', index: true },
  transitions: { type: [AgentRunTransitionSchema], default: [] },
  plan: mongoose.Schema.Types.Mixed,
  diffs: { type: [mongoose.Schema.Types.Mixed], default: [] },
  attempts: { type: [mongoose.Schema.Types.Mixed], default: [] },
  testResult: mongoose.Schema.Types.Mixed,
  securityFindings: { type: [mongoose.Schema.Types.Mixed], default: [] },
  checkpointId: { type: String, default: null },
  checkpointFiles: { type: [mongoose.Schema.Types.Mixed], default: [] }, // [{name, content}] snapshot for rollback
  error: { type: String, default: null },
  startedAt: { type: Date, default: Date.now },
  completedAt: { type: Date, default: null }
}, { versionKey: false });
AgentRunSchema.index({ ownerId: 1, projectId: 1, startedAt: -1 });
export const AgentRun = mongoose.models.BaristaAgentRun || mongoose.model('BaristaAgentRun', AgentRunSchema);
